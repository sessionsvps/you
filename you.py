...
async def logUserBot():
    load_dotenv()
    api_id = int(28771833)
    api_hash = "4a93e61bbf10011e599d5fbf380aa13b"
    phone_number = "51985518619"
    ...
    await client.send_message("@spm1251", f'<b>Bot encendido</b>', parse_mode="HTML")
    spammer_group = int("-4208947570")
    ...
    while True:
        ...
        try:
            await client.send_message("@spm1251", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)}</code>",parse_mode="HTML")
        except:
            pass
                ...
                if i['group_name'] not in ["Spam 2024","RESPALDO🇵🇪BINS PERU","➳𝒀𝑨𝑷𝑬 𝑫𝑬 𝑬𝑺𝑻𝑨𝑭𝑨𝑫𝑶𝑹𝑬𝑺 ✧","QUEMANDO ESTAFADORES","𝐏𝐄𝐑Ú 𝐀𝐘𝐔𝐃𝐀","Referencias Elmer 💸","🎭 CANAL MUNDO STREAMING PERÚ 🇵🇪","TU MARKETPLACE"]:
                    ...
                    for message_spam in messages_list:
                        ...
                        try:
                            ...
                            await client.send_message("@spm1251", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}',parse_mode="HTML")
                            resultado = False
                        if resultado:
                            await client.send_message("@spm1251", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>',parse_mode="HTML")  
                        await asyncio.sleep(90)
                        if j==1: break
            await client.send_message("@spm1251", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
